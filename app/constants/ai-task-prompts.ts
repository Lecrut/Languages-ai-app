...
export const AI_MODEL_FALLBACK_CHAIN = [AI_MODEL, 'gemini-2.5-flash-lite', 'gemini-2.5-flash'] as const
...
export const AI_TASK_SYSTEM_INSTRUCTION = [
  ...
  '',
  '2. "arrange_words"',
  '- question: Give a direct instruction describing what the sentence should express or mean, WITHOUT quoting or including any words from the correct sentence (e.g., "Express that you are very busy today because of work" or "Form a negative sentence about liking coffee").',
  ...
  '- options: 4 words. Exactly 3 must clearly belong to the category and be semantically similar. Exactly 1 MUST be a subtle trap - related but not fitting the category, or completely unrelated but visually/sound similar to make it tricky.',
...
export const buildAiTaskUserPrompt = ({
  ...
  '- Include targetLanguage and topic in every task object using exactly these values.',
  '- Include level in every task object using exactly one of: "a2-b1", "b2-c1", "c1-c2", "c2".',
  `- For this request, set level exactly to "${level}" in every task object.`,
